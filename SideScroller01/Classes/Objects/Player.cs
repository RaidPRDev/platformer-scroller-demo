using BloomPostprocess;
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;
using Microsoft.Xna.Framework.Input;
using SideScroller01.Classes.Objects.Enemies;
using SideScroller01.Classes.Objects.Items;
using SideScroller01.Classes.Scene;

namespace SideScroller01.Classes.Objects
{
    enum PlayerState
    {
        // Take Hit Cycle
        TakeHit,
        TakeHitLowKick,
        KnockedDown,
        Down,
        GettingUp,
        Dying,
        
        // Movement & Attacks
        Idle,
        Walking,
        Jumping,
        JumpKick,
        JumpFlyingKick,
        JumpFlyingPunch,
        Combo1Attack01,
        Combo1Attack02,
        Combo1Attack03,
        Combo2Attack01,
        Combo2Attack02,
        Combo2Attack03,
        ThrowRock,
        KickObject,
        PickUpItem,

        Level01Intro
        
    }
        
    class Player : Actor
    {
        public const float STARTING_HEALTH = 300;
        public float FrameWalkingSpeed = 0.5f;
        private const float MOVEMENT_THRESHOLD = 0.01f;
        
        private readonly float acceptCombo1Attack2Time = Actor.FrameRate * 3f;
        private readonly float acceptCombo1Attack3Time = Actor.FrameRate * 4f;
        private readonly float acceptCombo2Attack2Time = Actor.FrameRate * 5f;
        private readonly float acceptCombo2Attack3Time = Actor.FrameRate * 6f;

        #region draw area constants

        // WAIT - Not doing anything, show this one
        private const int DRAW_HEIGHT_WAIT = 122;
        private const int DRAW_WIDTH_WAIT = 96;

        private const int DRAW_HEIGHT_NORMAL = 122;
        private const int DRAW_WIDTH_IDLE = 96;
        private const int DRAW_WIDTH_WALK = 96;
        private const int DRAW_WIDTH_JUMP = 78;
        private const int DRAW_WIDTH_JUMPKICK = 137;

        private const int DRAW_HEIGHT_JUMP = 133;
        private const int DRAW_HEIGHT_JUMPKICK = 133;
        private const int DRAW_WIDTH_JUMPKICK2 = 137;
        private const int DRAW_HEIGHT_JUMPKICK2 = 133;
        private const int DRAW_WIDTH_JUMPPUNCH = 137;
        private const int DRAW_HEIGHT_JUMPPUNCH = 133;

        // Hit Game Item
        private const int DRAW_WIDTH_KICKITEM = 137;
        private const int DRAW_HEIGHT_KICKITEM = 136;

        // Pickup Game Item
        private const int DRAW_WIDTH_PICKUP = 139;
        private const int DRAW_HEIGHT_PICKUP = 133;

        // COMBO 01 - Punch - Punch - Punch
        private const int D_HEIGHT_COMBO1_ATTACK01 = 121;
        private const int D_HEIGHT_COMBO1_ATTACK02 = 121;
        private const int D_HEIGHT_COMBO1_ATTACK03 = 121;
        private const int D_WIDTH_COMBO1_ATTACK01 = 131;
        private const int D_WIDTH_COMBO1_ATTACK02 = 131;
        private const int D_WIDTH_COMBO1_ATTACK03 = 131;
        private const int D_ATTACK01_COMBO1_FRAME_Y = 1;
        private const int D_ATTACK02_COMBO1_FRAME_Y = 0;
        private const int D_ATTACK03_COMBO1_FRAME_Y = 3;

        // COMBO 02 - Kick - Dash Kick
        private const int D_HEIGHT_COMBO2_ATTACK01 = 136;
        private const int D_HEIGHT_COMBO2_ATTACK02 = 136;
        private const int D_HEIGHT_COMBO2_ATTACK03 = 136;
        private const int D_WIDTH_COMBO2_ATTACK01 = 131;
        private const int D_WIDTH_COMBO2_ATTACK02 = 131;
        private const int D_WIDTH_COMBO2_ATTACK03 = 131;
        private const int D_ATTACK01_COMBO2_FRAME_Y = 0;
        private const int D_ATTACK02_COMBO2_FRAME_Y = 0;
        private const int D_ATTACK03_COMBO2_FRAME_Y = 1;

        // Enemy
        private const int DRAW_WIDTH_TAKEHIT = 136;
        private const int DRAW_HEIGHT_TAKEHIT = 133;
        private const int DRAW_WIDTH_GETTINGUP = 137;
        private const int DRAW_WIDTH_KNOCKDOWN = 139;

        // Game Items
        private const int DRAW_WIDTH_ROCKTHROW = 139;

        #endregion

        private readonly PlayerIndex pIndex;

        public PlayerState State;

        // Can't use the normal position as this would also change the position of the shadow
        private Vector2 jumpingPos;

        // Need to know where the landing of the jump is
        private float landingHeight;

        // Attacks
        private float stateTime;
        private bool makeCombo1;
        private bool makeCombo2;
        public bool CarryingRock;

        // Need a reference so action on a GameItem can be delayed
        // until the correct frame of animation.
        GameItem aboutToUse; 

        // Cutscene
        Vector2 targetPosition;
               
        public Player(Vector2 position, Level inLevel, PlayerIndex pIndex)
            : base(position, inLevel)
        {
            this.pIndex = pIndex;
            this.CarryingRock = false;
            this.Health = STARTING_HEALTH;

            ResetIdleGraphic();
        }

        protected override void InitializeTextures()
        {
            base.InitializeTextures();

            Texture_Walk_Idle = TextureManager.GetTexure("CodySpriteWalkIdleSheet");
            Texture_Attack = TextureManager.GetTexure("CodySpriteAttackSheet");
            Texture_Attack_02 = TextureManager.GetTexure("CodySpriteAttack02Sheet");
            Texture_React = TextureManager.GetTexure("CodySpriteReactSheet");
        }

        public override void Update(GameTime gT)
        {
            switch (State)
            {
                #region Idle and Walking

                case PlayerState.Idle:
                case PlayerState.Walking:

                    #region Punch Attack
                    //if (InputHelper.WasKeyPressed(Keys.a))
                    if (InputHelper.WasButtonPressed(pIndex, Buttons.X)
                        || InputHelper.WasKeyPressed(Keys.NumPad7)
                        || InputHelper.WasKeyPressed(Keys.RightControl))
                    {
                        #region Any GameItems Here on Punch?

                        // Are we close enough to a GameItem to use?
                        for (int i = 0; i < InLevel.GameItems.Count; i++)
                        {
                            GameItem gameItem = InLevel.GameItems[i] as TrashCan;
                            if (gameItem != null)
                            {
                                if (gameItem.CheckCollision(this))
                                {
                                    // Kick It!
                                    // Setup animation
                                    this.aboutToUse = gameItem;
                                    this.State = PlayerState.KickObject;

                                    this.Sprite = Texture_Attack_02;
                                    this.CurrentFrameTime = 0;
                                    this.InitSpriteFrames(DRAW_WIDTH_KICKITEM, DRAW_HEIGHT_KICKITEM, 0, 2);
                                    this.SetOriginPosition("bottom");
                                    this.SetDrawArea();
                                    return;
                                }
                            }

                            gameItem = InLevel.GameItems[i] as PickUpStone;
                            if (gameItem != null)
                            {
                                if (gameItem.CheckCollision(this))
                                {
                                    PickUpItem(gameItem);
                                    return;
                                }
                            }

                            gameItem = InLevel.GameItems[i] as PickUpHealthPack;
                            if (gameItem != null)
                            {
                                if (gameItem.CheckCollision(this))
                                {
                                    PickUpItem(gameItem);
                                    return;
                                }
                            }
                        }
                        #endregion                        
                        
                        // Set Zoom/Bloom Effect
                        Game1.cam.SetZoom(1.15f, 20);
                        Game1.bloom.SetBloom(1.5f, 15f);

                        // Setup Draw Area
                        this.Sprite = Texture_Attack;
                        this.CurrentFrameTime = 0;
                        this.InitSpriteFrames(D_WIDTH_COMBO1_ATTACK01, D_HEIGHT_COMBO1_ATTACK01, 0, D_ATTACK01_COMBO1_FRAME_Y);
                        this.SetOriginPosition("bottom");
                        this.SetDrawArea();

                        this.stateTime = 0f;
                        this.State = PlayerState.Combo1Attack01;
                        this.makeCombo1 = false;

                        // Play Sound
                        SoundManager.PlaySound("Combo1-Hit", true);
                        return;

                    }
                    #endregion

                    #region Kick Attack
                    if (InputHelper.WasButtonPressed(pIndex, Buttons.Y)
                        || InputHelper.WasKeyPressed(Keys.NumPad8)
                        || InputHelper.WasKeyPressed(Keys.RightAlt))
                    {
                        #region Any GameItems Here?

                        // Are we close enough to a TrashCan to kick?
                        for (int i = 0; i < InLevel.GameItems.Count; i++)
                        {
                            GameItem gameItem = InLevel.GameItems[i] as TrashCan;
                            if (gameItem != null)
                            {
                                if (gameItem.CheckCollision(this))
                                {
                                    // Kick It!
                                    // Setup animation
                                    this.aboutToUse = gameItem;
                                    this.State = PlayerState.KickObject;

                                    this.Sprite = Texture_Attack_02;
                                    this.CurrentFrameTime = 0;
                                    this.InitSpriteFrames(DRAW_WIDTH_KICKITEM, DRAW_HEIGHT_KICKITEM, 0, 2);
                                    this.SetOriginPosition("bottom");
                                    this.SetDrawArea();
                                    return;
                                }
                            }
                        }
                        #endregion

                        // Setup Draw Area
                        this.Sprite = Texture_Attack_02;
                        this.InitSpriteFrames(D_WIDTH_COMBO2_ATTACK01, D_HEIGHT_COMBO2_ATTACK01, 0, D_ATTACK01_COMBO2_FRAME_Y);
                        this.SetDrawArea();
                      
                        if (FacingDir == DirectionTarget.Left) this.SetOriginPosition("bottom");
                        else this.SetOriginPosition("bottom");

                        this.stateTime = 0f;
                        this.State = PlayerState.Combo2Attack01;
                        this.makeCombo2 = false;

                        SoundManager.PlaySound("Attack04", true);
                        return;

                    }
                    #endregion

                    #region Jump
                    //if (InputHelper.WasKeyPressed(Keys.LeftShift))
                    if (InputHelper.WasButtonPressed(pIndex, Buttons.A)
                        || InputHelper.WasKeyPressed(Keys.NumPad4)
                        || InputHelper.WasKeyPressed(Keys.Space))
                    {
                        this.landingHeight = Position.Y;
                        this.jumpingPos = Position;
                        this.Speed.Y = -7; 
                        this.State = PlayerState.Jumping;

                        this.Sprite = Texture_React;
                        this.InitSpriteFrames(DRAW_WIDTH_JUMP, DRAW_HEIGHT_JUMP, 0, 0);
                        this.SetOriginPosition("bottom");
                        this.SetDrawArea();

                        SoundManager.PlaySound("Jump", true);
                        return;
                    }
                    #endregion

                    #region ThrowRocks

                    if (InputHelper.WasButtonPressed(this.pIndex, Buttons.B)
                        || InputHelper.WasKeyPressed(Keys.NumPad5)
                        || InputHelper.WasKeyPressed(Keys.Enter))
                    {
                        if (CarryingRock)
                        {
                            // Setup the animation
                            this.State = PlayerState.ThrowRock;
                            this.Sprite = Texture_React;
                            this.CurrentFrameTime = 0;
                            this.InitSpriteFrames(DRAW_WIDTH_ROCKTHROW, DRAW_HEIGHT_TAKEHIT, 0, 7);
                            this.SetOriginPosition("bottom");
                            this.SetDrawArea();
                            
                            SoundManager.PlaySound("Attack01", true);
                            return;
                        }
                    }

                    #endregion
                    
                    this.Move(gT);
                    break;

                #endregion

                #region Jumping

                case PlayerState.Jumping:
                    #region JumpKickAttack1
                    //if (InputHelper.WasKeyPressed(Keys.A))
                    if (InputHelper.WasButtonPressed(pIndex, Buttons.Y)
                        || InputHelper.WasKeyPressed(Keys.NumPad8)
                        || InputHelper.WasKeyPressed(Keys.RightAlt)) // Jump Kick
                    {
                        this.State = PlayerState.JumpKick;
                        this.InitSpriteFrames(DRAW_WIDTH_JUMPKICK, DRAW_HEIGHT_JUMPKICK, 0, 2);
                        this.SetOriginPosition("bottom");

                        SoundManager.PlaySound("Attack02", true);
                    }
                    //if (InputHelper.WasKeyPressed(Keys.G))
                    else if (InputHelper.WasButtonPressed(pIndex, Buttons.B)
                        || InputHelper.WasKeyPressed(Keys.NumPad5)
                        || InputHelper.WasKeyPressed(Keys.Enter)) // Side Flying Kick
                    {
                        this.State = PlayerState.JumpFlyingKick;
                        this.InitSpriteFrames(DRAW_WIDTH_JUMPKICK2, DRAW_HEIGHT_JUMPKICK2, 0, 3);
                        this.SetOriginPosition("bottom");
                     
                        SoundManager.PlaySound("GetOutOfMyFace");
                        SoundManager.PlaySound("Attack03", true);
                    }
                    else if (InputHelper.WasButtonPressed(pIndex, Buttons.X)
                        || InputHelper.WasKeyPressed(Keys.NumPad7)) // Flying Jump Punch
                    {
                        this.State = PlayerState.JumpFlyingPunch;
                        this.InitSpriteFrames(DRAW_WIDTH_JUMPPUNCH, DRAW_HEIGHT_JUMPPUNCH, 0, 4);
                        this.SetOriginPosition("bottom");

                        SoundManager.PlaySound("ThrowPunch", true);
                    }

                    #endregion

                    this.jumpingPos += this.Speed;
                    this.Position.X = this.jumpingPos.X;
                    this.Speed.Y += 0.3f; // Slow Verticle speed down because of gravity

                    if (this.jumpingPos.Y >= this.landingHeight) // Have we landed?
                    {
                        this.ResetIdleGraphic();
                        this.Speed.Y = 0;
                        break;
                    }

                    this.ConstrainToScreen();
                    if (this.State == PlayerState.JumpKick)
                    {
                        AnimateJumpKick(gT);
                    }
                    else if (this.State == PlayerState.JumpFlyingKick)
                    {
                        AnimateFlyingJumpKick(gT);
                    }
                    else
                    {
                        AnimateJumping(gT);
                    }
                    break;

                #endregion

                #region JumpKick

                case PlayerState.JumpKick:
                    this.jumpingPos += this.Speed;
                    this.Position.X = this.jumpingPos.X;
                    this.Speed.Y += 0.2f; // Slow Verticle speed down because of gravity when kicking
                    if (this.jumpingPos.Y >= this.landingHeight) // Have we landed?
                    {
                        this.ResetIdleGraphic();
                        this.Speed.Y = 0;
                        break;
                    }

                    this.ConstrainToScreen();
                    AnimateJumpKick(gT);    
                    break;

                #endregion

                #region JumpPunch

                case PlayerState.JumpFlyingPunch:
                    this.jumpingPos += this.Speed;
                    this.Position.X = this.jumpingPos.X;
                    this.Speed.Y += 0.2f; // Slow Verticle speed down because of gravity when kicking
                    if (this.jumpingPos.Y >= this.landingHeight) // Have we landed?
                    {
                        this.ResetIdleGraphic();
                        this.Speed.Y = 0;
                        break;
                    }

                    this.ConstrainToScreen();
                    AnimateJumpPunch(gT);
                    break;

                #endregion

                #region JumpFlyingKick

                case PlayerState.JumpFlyingKick:
                    this.jumpingPos += this.Speed;
                    this.Position.X = this.jumpingPos.X;
                    this.Speed.Y += 0.2f; // Slow Verticle speed down because of gravity when kicking
                    if (this.jumpingPos.Y >= this.landingHeight) // Have we landed?
                    {
                        this.ResetIdleGraphic();
                        this.Speed.Y = 0;
                        break;
                    }

                    this.ConstrainToScreen();
                    AnimateFlyingJumpKick(gT);
                    break;

                #endregion

                #region Combo1PunchAttack01
                case PlayerState.Combo1Attack01:

                    this.stateTime += (float)gT.ElapsedGameTime.TotalSeconds;
                    if (this.stateTime < acceptCombo1Attack2Time) // Still time to make a combo
                    {
                        //|| InputHelper.WasKeyPressed(Keys.A)
                        if (InputHelper.WasButtonPressed(pIndex, Buttons.X)
                            || InputHelper.WasKeyPressed(Keys.NumPad7)
                            || InputHelper.WasKeyPressed(Keys.RightControl))
                        {
                            this.makeCombo1 = true;
                            Game1.cam.SetZoom(1.25f, 20);
                            Game1.bloom.SetBloom(1.9f, 15f);

                        }
                        else if (InputHelper.WasButtonPressed(pIndex, Buttons.Y)
                            || InputHelper.WasKeyPressed(Keys.NumPad8)
                            || InputHelper.WasKeyPressed(Keys.RightAlt)
                            && this.FrameX > 2)
                            // If Punching for Combo, Check to see if player pressed Kick button after
                            // If, so change animation to kick
                        {
                            this.makeCombo1 = false;

                            this.Sprite = Texture_Attack_02;
                            this.InitSpriteFrames(D_WIDTH_COMBO2_ATTACK01, D_HEIGHT_COMBO2_ATTACK01, 0, D_ATTACK01_COMBO2_FRAME_Y);
                            this.SetOriginPosition("bottom");
                            this.SetDrawArea();
                            this.stateTime = 0f;
                            this.State = PlayerState.Combo2Attack01;

                            SoundManager.PlaySound("Combo2-Hit", true);
                        }
                    }

                    else // stateTime IS bigger than acceptAttack2Time
                    {
                        if (makeCombo1)
                        {
                            // Setup draw area
                            this.InitSpriteFrames(D_WIDTH_COMBO1_ATTACK02, D_HEIGHT_COMBO1_ATTACK02, 3, D_ATTACK02_COMBO1_FRAME_Y);
                            this.SetOriginPosition("bottom");
                            this.SetDrawArea();

                            // update input times
                            this.stateTime = 0f;
                            this.State = PlayerState.Combo1Attack02;
                            this.makeCombo1 = false;
                            
                            SoundManager.PlaySound("TakeThat");
                            SoundManager.PlaySound("Combo2-Hit", true);
                            return;
                        }
                    }

                    AnimateCombo1Attack01(gT);
                    break;
                #endregion

                #region Combo1PunchAttack02
                case PlayerState.Combo1Attack02:

                    stateTime += (float)gT.ElapsedGameTime.TotalSeconds;
                    if (stateTime < acceptCombo1Attack3Time) // Still time to make a combo
                    {
                        //if (InputHelper.WasKeyPressed(Keys.A)) 
                        if (InputHelper.WasButtonPressed(pIndex, Buttons.X)
                            || InputHelper.WasKeyPressed(Keys.NumPad7)
                            || InputHelper.WasKeyPressed(Keys.RightControl))
                        {
                            this.makeCombo1 = true;
                            Game1.cam.SetZoom(1.45f, 20);
                            Game1.bloom.SetBloom(2.4f, 15f);
                        }
                    }

                    else // stateTime IS bigger than acceptAttack2Time
                    {
                        if (this.makeCombo1)
                        {
                            // Setup draw area
                            this.InitSpriteFrames(D_WIDTH_COMBO1_ATTACK03, D_HEIGHT_COMBO1_ATTACK03, 0, D_ATTACK03_COMBO1_FRAME_Y);
                            this.SetOriginPosition("bottom");
                            this.SetDrawArea();

                            // Set Attack-Input times
                            this.stateTime = 0f;
                            this.State = PlayerState.Combo1Attack03;
                            this.makeCombo1 = false;

                            SoundManager.PlaySound("Combo3-Hit", true);
                            return;
                        }
                    }

                    AnimateCombo1Attack02(gT);
                    break;


                #endregion

                #region Combo1PunchAttack03

                case PlayerState.Combo1Attack03:
                    AnimateCombo1Attack03(gT);
                    break;

                #endregion

                #region Combo2KickAttack01
                case PlayerState.Combo2Attack01:

                    this.stateTime += (float)gT.ElapsedGameTime.TotalSeconds;
                    if (this.stateTime < acceptCombo2Attack2Time) // Still time to make a combo
                    {
                        //if (InputHelper.WasKeyPressed(Keys.Z)) 
                        if (InputHelper.WasButtonPressed(pIndex, Buttons.Y)
                            || InputHelper.WasKeyPressed(Keys.NumPad8)
                            || InputHelper.WasKeyPressed(Keys.RightAlt))
                        {
                            this.makeCombo2 = true;
                        }
                        //else if (InputHelper.WasKeyPressed(Keys.D)) 
                        else if (InputHelper.WasButtonPressed(pIndex, Buttons.X)
                            || InputHelper.WasKeyPressed(Keys.NumPad7)
                            || InputHelper.WasKeyPressed(Keys.RightControl) 
                            && this.FrameX >= 3)
                        // If Kicking for Combo, Check to see if player pressed Punch button after
                        // If, so change animation to punch
                        {
                            this.Sprite = Texture_Attack;
                            this.InitSpriteFrames(D_WIDTH_COMBO1_ATTACK01, D_HEIGHT_COMBO1_ATTACK01, 0, D_ATTACK01_COMBO1_FRAME_Y);
                            this.SetDrawArea();

                            if (FacingDir == DirectionTarget.Left)
                                this.SetOriginPosition("bottom", 18, 2);
                                // this.Origin = new Vector2((this.DrawWidth / 2) + 18, this.DrawHeight + 2);
                            else // Right
                                this.SetOriginPosition("bottom", -18, 2);
                                // this.Origin = new Vector2((this.DrawWidth / 2) - 18, this.DrawHeight + 2);
                     
                            this.stateTime = 0f;
                            this.State = PlayerState.Combo1Attack01;
                            this.makeCombo1 = false;
                           
                            SoundManager.PlaySound("FinishHit");
                            return;
                        }
                    }

                    else // stateTime IS bigger than acceptAttack2Time
                    {
                        if (makeCombo2)
                        {
                            // Setup draw area
                            this.InitSpriteFrames(D_WIDTH_COMBO2_ATTACK02, D_HEIGHT_COMBO2_ATTACK02, 6, D_ATTACK02_COMBO2_FRAME_Y);
                            this.SetOriginPosition("bottom");
                            this.SetDrawArea();

                            // Set Zoom/Bloom Effect
                            Game1.cam.SetZoom(1.15f, 20);
                            Game1.bloom.SetBloom(1.5f, 15f);

                            // Set Attack-Input times
                            this.stateTime = 0f;
                            this.State = PlayerState.Combo2Attack02;
                            this.makeCombo2 = false;

                            SoundManager.PlaySound("Attack03", true);
                            SoundManager.PlaySound("FinishHit");
                            return;
                        }
                    }

                    AnimateCombo2Attack01(gT);
                    break;


                #endregion

                #region Combo2KickAttack02

                case PlayerState.Combo2Attack02:
                    AnimateCombo2Attack02(gT);
                    break;

                #endregion

                #region Rock Throw

                case PlayerState.ThrowRock:
                    AnimateThrowRock(gT);
                    break;

                #endregion

                #region TrashCan

                case PlayerState.KickObject:
                    AnimateKickObject(gT);
                    break;

                #endregion

                #region PickUpItems
                case PlayerState.PickUpItem:
                    AnimatePickUpItem(gT);
                    break;
                #endregion

                #region Take Hit and Die Cycle
                case PlayerState.TakeHit:
                    AnimateTakeHit(gT);
                    break;

                case PlayerState.KnockedDown:
                    AnimateKnockDown(gT);
                    break;

                case PlayerState.Down:
                    this.stateTime += (float)gT.ElapsedGameTime.TotalSeconds;
                    if (this.stateTime >= Actor.DOWN_TIME)
                    {
                        // Set up Gettign Up Animation
                        State = PlayerState.GettingUp;
                        this.CurrentFrameTime = 0;
                        this.InitSpriteFrames(DRAW_WIDTH_GETTINGUP, DRAW_HEIGHT_TAKEHIT, 0, 5);
                        this.SetOriginPosition("bottom");
                        this.SetDrawArea();
                    }
                    break;

                case PlayerState.GettingUp:
                    //IsVisible = true;
                    this.stateTime -= (float)gT.ElapsedGameTime.TotalSeconds;
                    if (this.stateTime <= 0)
                    {
                        if (this.GettingUpFlashes <= 50)
                        {
                            this.stateTime = PLAYER_DEATH_FLASH_TIME;
                            this.IsVisible = !this.IsVisible;
                            this.GettingUpFlashes++;
                        }
                    }

                    AnimateGettingUp(gT);
                    break;

                case PlayerState.Dying:
                    // Flash the Body a few times
                    this.stateTime -= (float)gT.ElapsedGameTime.TotalSeconds;
                    if (this.stateTime <= 0)
                    {
                        this.stateTime = PLAYER_DEATH_FLASH_TIME;
                        this.IsVisible = !this.IsVisible;
                        this.DeathFlashes++;

                        // Actor is Dead
                        if (this.DeathFlashes <= 8)
                        {
                            this.InLevel.LevelState = LevelState.Continue;
                            this.InLevel.TimerContinue = 11f - (float)gT.ElapsedGameTime.TotalSeconds;
                            HUDManager.Opacity = 0f;
                        }
                    }
                    break;

                #endregion

                #region Level Intro

                case PlayerState.Level01Intro:
                    if (this.Position != targetPosition)
                    {
                        // Move to the destination
                        LineUpXTargetPos();
                        LineUpYTargetPos();

                        AnimateWalking(gT);
                    }
                    else
                    {
                        this.ResetIdleGraphic();
                        this.InLevel.LevelState = LevelState.CutScene;
                        this.InLevel.CutScenes[InLevel.CurrentCutScene].PlayFirstLine();
                    }
                    break;


                #endregion
            }
        }

        public override void Draw(SpriteBatch SB)
        {
            Vector2 pos = GameManager.Levels[GameManager.CurrentLevel].Player1.Position;

            if (this.IsVisible)
            {
                // Draw Character
                // Are we jumping
                if (this.State == PlayerState.Jumping || this.State == PlayerState.JumpKick
                    || this.State == PlayerState.JumpFlyingKick
                    || this.State == PlayerState.JumpFlyingPunch)
                {
                    // Facing Left or Right?
                    if (FacingDir == DirectionTarget.Right)
                        SB.Draw(this.Sprite, Camera.GetScreenPosition(jumpingPos), this.DrawArea,
                            Color.White, 0f, this.Origin, 1f, SpriteEffects.None, LayerDepth);
                    else // We must be facing to the left
                        SB.Draw(this.Sprite, Camera.GetScreenPosition(jumpingPos), this.DrawArea,
                            Color.White, 0f, this.Origin, 1f, SpriteEffects.FlipHorizontally, LayerDepth);
                }
                else
                {
                    // Facing Left or Right?
                    if (this.FacingDir == DirectionTarget.Right)
                        SB.Draw(this.Sprite, Camera.GetScreenPosition(Position), this.DrawArea,
                            Color.White, 0f, this.Origin, 1f, SpriteEffects.None, LayerDepth);
                    else // We must be facing to the left
                        SB.Draw(this.Sprite, Camera.GetScreenPosition(Position), this.DrawArea,
                            Color.White, 0f, this.Origin, 1f, SpriteEffects.FlipHorizontally, LayerDepth);
                }
            }

            base.Draw(SB);
        }

        private void GetXSpeed(float magnitude)
        {
            if (magnitude <= 0.25f) /*sneak*/
            {
                this.FrameWalkingSpeed = 1.3f;
                if (this.FacingDir == DirectionTarget.Left)
                    this.Speed.X = -0.2f;
                else
                    this.Speed.X = 0.2f;
            }
            else if (magnitude <= 0.75f) /*walk*/
            {
                this.FrameWalkingSpeed = 0.8f;
                if (FacingDir == DirectionTarget.Left)
                    this.Speed.X = -1f;
                else
                    this.Speed.X = 1f;
            }
            else /*run*/
            {
                this.FrameWalkingSpeed = 0.5f;
                if (FacingDir == DirectionTarget.Left)
                    this.Speed.X = -2.5f;
                else
                    this.Speed.X = 2.5f;
            }
        }

        private void GetYSpeed(bool nDir, float magnitude)
        {
            if (magnitude <= 0.25f) /*sneak*/
            {
                this.FrameWalkingSpeed = 1.3f;
                if (nDir)
                    this.Speed.Y = -0.2f;
                else
                    this.Speed.Y = 0.2f;

            }
            else if (magnitude <= 0.75f) /*walk*/
            {
                this.FrameWalkingSpeed = 0.8f;
                if (nDir)
                    this.Speed.Y = -1f;
                else
                    this.Speed.Y = 1f;
            }
            else /*run*/
            {
                this.FrameWalkingSpeed = 0.5f;
                if (nDir)
                    this.Speed.Y = -2f;
                else
                    this.Speed.Y = 2f;
            }
        }

        private void Move(GameTime gT)
        {
            this.FrameWalkingSpeed = 1f;

            #region X Movement
            if (InputHelper.IsKeyHeld(Keys.Left) || InputHelper.IsKeyHeld(Keys.A)
                || InputHelper.IsButtonHeld(pIndex, Buttons.DPadLeft))
            {
                this.FacingDir = DirectionTarget.Left;
                this.GetXSpeed(0.99f);
                this.State = PlayerState.Walking;
                this.DrawWidth = DRAW_WIDTH_WALK;
                this.DrawHeight = DRAW_HEIGHT_NORMAL;
            }
            else if (InputHelper.NGS[(int)pIndex].ThumbSticks.Left.X < -MOVEMENT_THRESHOLD)
            {
                this.FacingDir = DirectionTarget.Left;
                this.GetXSpeed(InputHelper.NGS[(int)pIndex].ThumbSticks.Left.Length());
                this.State = PlayerState.Walking;
                this.DrawWidth = DRAW_WIDTH_WALK;
                this.DrawHeight = DRAW_HEIGHT_NORMAL;
            }
            else if (InputHelper.IsKeyHeld(Keys.Right) || InputHelper.IsKeyHeld(Keys.D)
                || InputHelper.IsButtonHeld(pIndex, Buttons.DPadRight))
            {
                this.FacingDir = DirectionTarget.Right;
                this.GetXSpeed(0.76f);
                this.State = PlayerState.Walking;
                this.DrawWidth = DRAW_WIDTH_WALK;
                this.DrawHeight = DRAW_HEIGHT_NORMAL;
            }

            else if (InputHelper.NGS[(int)pIndex].ThumbSticks.Left.X > MOVEMENT_THRESHOLD)
            {
                this.FacingDir = DirectionTarget.Right;
                this.GetXSpeed(InputHelper.NGS[(int)pIndex].ThumbSticks.Left.Length());
                this.State = PlayerState.Walking;
                this.DrawWidth = DRAW_WIDTH_WALK;
                this.DrawHeight = DRAW_HEIGHT_NORMAL;
            }
            else
            {
                this.Speed.X = 0;
            }
            #endregion

            #region Y Movement
            if (InputHelper.IsKeyHeld(Keys.Down) || InputHelper.IsKeyHeld(Keys.S)
                || InputHelper.IsButtonHeld(pIndex, Buttons.DPadDown))
            {
                // float magnitude = InputHelper.NGS[(int)pIndex].ThumbSticks.Left.Length();
                this.GetYSpeed(false, 0.99f);
                this.State = PlayerState.Walking;
                this.DrawWidth = DRAW_WIDTH_WALK;
                this.DrawHeight = DRAW_HEIGHT_NORMAL;
            }
            else if (InputHelper.NGS[(int)pIndex].ThumbSticks.Left.Y < -MOVEMENT_THRESHOLD)
            {
                this.GetYSpeed(false, InputHelper.NGS[(int)pIndex].ThumbSticks.Left.Length());
                this.State = PlayerState.Walking;
                this.DrawWidth = DRAW_WIDTH_WALK;
                this.DrawHeight = DRAW_HEIGHT_NORMAL;
            }
            else if (InputHelper.IsKeyHeld(Keys.Up) || InputHelper.IsKeyHeld(Keys.W)
                || InputHelper.IsButtonHeld(pIndex, Buttons.DPadUp))
            {
                this.GetYSpeed(true, 0.99f);
                this.State = PlayerState.Walking;
                this.DrawWidth = DRAW_WIDTH_WALK;
                this.DrawHeight = DRAW_HEIGHT_NORMAL;
            }
            else if (InputHelper.NGS[(int)pIndex].ThumbSticks.Left.Y > MOVEMENT_THRESHOLD)
            {
                this.GetYSpeed(true, InputHelper.NGS[(int)pIndex].ThumbSticks.Left.Length());
                this.State = PlayerState.Walking;
                this.DrawWidth = DRAW_WIDTH_WALK;
                this.DrawHeight = DRAW_HEIGHT_NORMAL;
            }
            else
            {
                this.Speed.Y = 0;
            }
            #endregion

            if (this.Speed == Vector2.Zero)
            {
                // Make Character AnimateIdle()
                this.InitSpriteFrames(DRAW_WIDTH_IDLE, DRAW_HEIGHT_NORMAL, this.FrameX, 0);
                this.State = PlayerState.Idle;
                this.SetOriginPosition("bottom");
                this.SetDrawArea();

                AnimateIdle(gT);
            }
            else // that means we are moving
            {
                this.Position += this.Speed;
                this.GetLayerDepth(this.Position.Y);
                this.ConstrainToScreen();

                AnimateWalking(gT);
            }
        }

        /// <summary>
        /// Does not let the player leave the screen
        /// </summary>
        private void ConstrainToScreen()
        {
            // Don't let the player out of the screen
            // Or out of playBounds
            if (Position.X < Camera.Position.X - Game1.SCREEN_WIDTH)
                Position.X = Camera.Position.X - Game1.SCREEN_WIDTH;

            if (Position.X > Camera.Position.X + Game1.SCREEN_WIDTH)
                Position.X = Camera.Position.X + Game1.SCREEN_WIDTH;

            if (Position.Y < InLevel.PlayBounds.Top) Position.Y = InLevel.PlayBounds.Top;
            if (Position.Y > InLevel.PlayBounds.Bottom) Position.Y = InLevel.PlayBounds.Bottom;
        }

        public void GetStone()
        {
            this.CarryingRock = true;
        }

        public void GetHealth(int howMuch)
        {
            this.Health += howMuch;

            if (this.Health > STARTING_HEALTH) this.Health = STARTING_HEALTH;
        }

        private void PickUpItem(GameItem gameItem)
        {
            // Setup animation
            this.aboutToUse = gameItem;
            this.State = PlayerState.PickUpItem;
            this.Sprite = Texture_React;
            this.CurrentFrameTime = 0;
            this.InitSpriteFrames(DRAW_WIDTH_PICKUP, DRAW_HEIGHT_PICKUP, 0, 7);
            this.SetOriginPosition("bottom");
            this.SetDrawArea();
        }

        private void ThrowRocks()
        {
            // Throw a rock 
            this.InLevel.GameItems.Add(new Rock(this.Position, this.FacingDir, this.InLevel, this));

            this.CarryingRock = false;

            // Play Rock Throw Sound
            SoundManager.PlaySound("TakeThat");
        }

        public void Continue()
        {
            this.Health = STARTING_HEALTH;

            // Set up the getting up animation
            this.State = PlayerState.GettingUp;
            this.Sprite = Texture_React;
            this.CurrentFrameTime = 0;
            this.InitSpriteFrames(DRAW_WIDTH_GETTINGUP, DRAW_HEIGHT_TAKEHIT, 0, 5);
            this.SetOriginPosition("bottom");
            this.SetDrawArea();
        }

        public void SetIntro01TargetPosition(Vector2 targetPosition)
        {
            this.targetPosition = targetPosition;
            this.State = PlayerState.Level01Intro;
            this.InLevel.LevelState = LevelState.Intro;
            
            // Setup Walk Animation
            this.Sprite = Texture_Walk_Idle;
            this.CurrentFrameTime = 0;
            this.InitSpriteFrames(DRAW_WIDTH_WALK, DRAW_HEIGHT_NORMAL, 0 , 1);
            this.SetOriginPosition("bottom");
            this.SetDrawArea();
        }

        private void LineUpXTargetPos  ()
        {
            // is the targetPosition to the left
            if (targetPosition.X < this.Position.X) // TargetPosition is to the left
            {
                // Move left
                this.Position.X -= 3f;
                FacingDir = DirectionTarget.Left;
                if (targetPosition.X >= this.Position.X)
                {
                    this.Position.X = targetPosition.X;
                }
            }
            else if (targetPosition.X > this.Position.X) // TargetPosition is to the right
            {
                // Move Right
                this.Position.X += 3f;
                FacingDir = DirectionTarget.Right;
                if (targetPosition.X <= this.Position.X)
                {
                    this.Position.X = targetPosition.X;
                }
            }
        }

        private void LineUpYTargetPos()
        {
            // is the targetPosition above or below
            if (targetPosition.Y < this.Position.Y) // TargetPosition is above
            {
                // Move Up
                this.Position.Y -= 2f;
                if (targetPosition.Y >= this.Position.Y)
                {
                    this.Position.Y = targetPosition.Y;
                }
            }
            else if (targetPosition.Y > this.Position.Y) // TargetPosition is below
            {
                // Move Down
                this.Position.Y += 2f;
                if (targetPosition.Y <= this.Position.Y)
                {
                    this.Position.Y = targetPosition.Y;
                }
            }
        }

        #region Collision Detection

        public override void UpdateHitArea()
        {
            this.HitArea = this.DrawWidth / 2;
        }

        private void CheckEnemyCollision()
        {
            this.UpdateHitArea();

            for (int i = InLevel.Actors.Count - 1; i >= 0; i--)
            {
                Actor actor;

                // Make sure our not looking at ourself
                actor = InLevel.Actors[i] as Player;
                if (actor == this)
                    continue;

                // Are we facing ROLENTO enemy?
                actor = InLevel.Actors[i] as RolentoEnemy;
                if (actor != null)
                {
                    // Update the current actors Hit Area
                    actor.UpdateHitArea();

                    // 1) Is Actor/Enemy attackable?
                    if (actor.IsAttackable)
                    {
                        // 2) Are wer within Y Range
                        if (actor.Position.Y > this.Position.Y - HIT_Y_RANGE
                            && actor.Position.Y < this.Position.Y + HIT_Y_RANGE)
                        {
                            // 3) Which way is the player facing
                            if (this.FacingDir == DirectionTarget.Left)
                            {
                                // 4) Is the enemy/actor in front of us **LEFT**
                                if (actor.Position.X < this.Position.X)
                                {
                                    // 5) Players left edge <MORE LEFT> than actors RIGHT
                                    if (this.Position.X - HitArea < actor.Position.X + actor.HitArea)
                                    {
                                        HitSomeone(actor);
                                    }
                                }
                            }
                            //  3) Which way is the player facing
                            else
                            {
                                // A) is Actor in in front of us or RIGHT OF the player
                                if (actor.Position.X > this.Position.X)
                                {
                                    // 5) Players RIGHT EDGE is more right than actor's LEFT 
                                    if (this.Position.X + HitArea > actor.Position.X - actor.HitArea)
                                    {
                                        HitSomeone(actor);
                                    }
                                }
                            }
                        }
                    }
                }
                
                // Are we facing enemy?
                actor = InLevel.Actors[i] as DeejayEnemy;
                if (actor != null)
                {
                    // Update the current actors Hit Area
                    actor.UpdateHitArea();

                    // 1) Is Actor/Enemy attackable?
                    if (actor.IsAttackable)
                    {
                        // 2) Are wer within Y Range
                        if (actor.Position.Y > this.Position.Y - HIT_Y_RANGE
                            && actor.Position.Y < this.Position.Y + HIT_Y_RANGE)
                        {
                            // 3) Which way is the player facing
                            if (this.FacingDir == DirectionTarget.Left)
                            {
                                // 4) Is the enemy/actor in front of us **LEFT**
                                if (actor.Position.X < this.Position.X)
                                {
                                    // 5) Players left edge <MORE LEFT> than actors RIGHT
                                    if (this.Position.X - HitArea < actor.Position.X + actor.HitArea)
                                    {
                                        HitSomeone(actor);
                                    }
                                }
                            }
                            //  3) Which way is the player facing
                            else
                            {
                                // A) is Actor in in front of us or RIGHT OF the player
                                if (actor.Position.X > this.Position.X)
                                {
                                    // 5) Players RIGHT EDGE is more right than actor's LEFT 
                                    if (this.Position.X + HitArea > actor.Position.X - actor.HitArea)
                                    {
                                        HitSomeone(actor);
                                    }
                                }
                            }
                        }
                    }

                }

                // Are we facing enemy?
                actor = InLevel.Actors[i] as AdonEnemy;
                if (actor != null)
                {
                    // Update the current actors Hit Area
                    actor.UpdateHitArea();

                    // 1) Is Actor/Enemy attackable?
                    if (actor.IsAttackable)
                    {
                        // 2) Are wer within Y Range
                        if (actor.Position.Y > this.Position.Y - HIT_Y_RANGE
                            && actor.Position.Y < this.Position.Y + HIT_Y_RANGE)
                        {
                            // 3) Which way is the player facing
                            if (this.FacingDir == DirectionTarget.Left)
                            {
                                // 4) Is the enemy/actor in front of us **LEFT**
                                if (actor.Position.X < this.Position.X)
                                {
                                    // 5) Players left edge <MORE LEFT> than actors RIGHT
                                    if (this.Position.X - HitArea < actor.Position.X + actor.HitArea)
                                    {
                                        HitSomeone(actor);
                                    }
                                }
                            }
                            //  3) Which way is the player facing
                            else
                            {
                                // A) is Actor in in front of us or RIGHT OF the player
                                if (actor.Position.X > this.Position.X)
                                {
                                    // 5) Players RIGHT EDGE is more right than actor's LEFT 
                                    if (this.Position.X + HitArea > actor.Position.X - actor.HitArea)
                                    {
                                        HitSomeone(actor);
                                    }
                                }
                            }
                        }
                    }
                }
            }
        }

        private void SetEffect(string effect, Actor who, Player p) 
        {
            // Display Small Spark Effect on Hit
            SpecialEffects spark = new SpecialEffects(effect, who.Position, InLevel, p);
            InLevel.GameItems.Add(spark);  
        }

        private void HitSomeone(Actor whoToHit)
        {
            switch (State)
            {
                case PlayerState.Combo1Attack01: // Straight Jab
                    SetEffect("smallspark", whoToHit, this);
                    whoToHit.GetHit(FacingDir, 15);
                    break;
  
                case PlayerState.Combo1Attack02: // Upper Cut
                    SetEffect("bluespark", whoToHit, this);
                    whoToHit.GetHit(FacingDir, 25);
                    break;

                case PlayerState.Combo1Attack03: // Side Kick
                    SetEffect("smallspark", whoToHit, this);
                    whoToHit.GetKnockedDown(FacingDir, 35);
                    break;

                case PlayerState.Combo2Attack01: // Front Kick 
                    SetEffect("smallspark", whoToHit, this);
                    whoToHit.GetHitKick(FacingDir, 30);
                    break;

                case PlayerState.Combo2Attack02: // Super Slide Kick
                    SetEffect("bluespark", whoToHit, this);
                    whoToHit.GetKnockedDown(FacingDir, 50);
                    break;

                case PlayerState.JumpKick: // Super Slide Kick
                    SetEffect("smallspark", whoToHit, this);
                    whoToHit.GetHit(FacingDir, 50);
                    break;

                case PlayerState.JumpFlyingKick: // Super Slide Kick
                    SetEffect("bluespark", whoToHit, this);
                    whoToHit.GetKnockedDown(FacingDir, 50);
                    break;

                case PlayerState.JumpFlyingPunch: // Super Slide Punch
                    SetEffect("smallspark", whoToHit, this);
                    whoToHit.GetHit(FacingDir, 40);
                    break;
            }
        }

        public override void GetHit(DirectionTarget cameFrom, int damage)
        {
            this.Health -= damage;
            if (CheckForDeath())
            {
                SoundManager.PlaySound("GetHit-Died", true);
                GetKnockedDown(cameFrom, 0);
                return;
            }

            // Set state and tetxture
            this.State = PlayerState.TakeHit;
            this.Sprite = Texture_React;
            this.InitSpriteFrames(DRAW_WIDTH_TAKEHIT, DRAW_HEIGHT_TAKEHIT, 0, 6);
            this.SetOriginPosition("bottom");
            this.SetDrawArea();
  
            // Play Sound
            SoundManager.PlaySound("GetHit01", true);

            // Face the man who beat you
            if (cameFrom == DirectionTarget.Left) FacingDir = DirectionTarget.Right;
            else FacingDir = DirectionTarget.Left;
        }

        public override void GetKnockedDown(DirectionTarget cameFrom, int damage)
        {
            // Set state and texture
            this.IsAttackable = false;
            this.State = PlayerState.KnockedDown;
            this.Sprite = Texture_React;
            this.InitSpriteFrames(DRAW_WIDTH_KNOCKDOWN, DRAW_HEIGHT_TAKEHIT, 0, 1);
            this.SetOriginPosition("bottom");
            this.SetDrawArea();

            SoundManager.PlaySound("GetHit03", true);

            Game1.bloomIntensityIndex = BloomSettings.BloomIntensityNormal;

            // Face the man who beat you
            if (cameFrom == DirectionTarget.Right) FacingDir = DirectionTarget.Left;
            else FacingDir = DirectionTarget.Right;

            this.Health -= damage;
        }

        #endregion

        #region Animations

        /// <summary>
        /// Resets player to it's idle state, awaiting player input
        /// </summary>
        public void ResetIdleGraphic()
        {
            Game1.cam.SetZoom(1f, 7);
            Game1.bloomIntensityIndex = BloomSettings.BloomIntensityNormal;

            this.IsAttackable = true;
            this.Sprite = Texture_Walk_Idle;
            this.State = PlayerState.Idle;
            this.CurrentFrameTime = 0;
            this.InitSpriteFrames(DRAW_WIDTH_IDLE, DRAW_HEIGHT_NORMAL, 0, 0);
            this.SetDrawArea();
            this.SetOriginPosition("bottom");
            this.UpdateHitArea();
        }

        private void AnimateIdle(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;// *.9f;
            if (this.CurrentFrameTime >= Actor.FrameRate * 0.8f)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;
                if (this.FrameX > 7) this.FrameX = 0;

                this.SetDrawArea();
            }
        }

        private void AnimateWalking(GameTime gT)
        {
            this.SetOriginPosition("bottom");
            
            this.FrameY = 1;
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;

            if (this.CurrentFrameTime >= Actor.FrameRate * FrameWalkingSpeed)
            {
                this.CurrentFrameTime = 0;
                this.FrameX++;

                if (this.FrameY == 1)
                {
                    if (this.FrameX > 9)
                    {
                        this.FrameX = 0;
                    }
                }
                else // FrameY must be equal to 2
                {
                    if (this.FrameX > 9)
                    {
                        this.FrameX = 0;
                    }
                }

                SetDrawArea();
            }
        }

        private void AnimateJumping(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;
            if (this.CurrentFrameTime >= Actor.FrameRate)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;
                if (this.FrameX > 7)
                {
                    this.FrameX = 7;
                    return;
                }

                SetDrawArea();
            }
        }

        private void AnimateJumpKick(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;
            if (this.CurrentFrameTime >= Actor.FrameRate * 0.5f)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                if (this.FrameX > 6)
                {
                    this.Sprite = Texture_React;
                    this.State = PlayerState.Jumping;
                    this.InitSpriteFrames(DRAW_WIDTH_JUMP, DRAW_HEIGHT_JUMP, 1, 0);
                    this.SetOriginPosition("bottom");
                }

                // Collision Detection
                if (this.FrameX == 4) this.CheckEnemyCollision();

                SetDrawArea();
            }
        }

        private void AnimateJumpPunch(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;
            if (this.CurrentFrameTime >= Actor.FrameRate)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                if (this.FrameX > 3)
                {
                    this.Sprite = Texture_React;
                    this.State = PlayerState.Jumping;
                    this.InitSpriteFrames(DRAW_WIDTH_JUMP, DRAW_HEIGHT_JUMP, 0, 1);
                    this.SetOriginPosition("bottom");
                }
                // Collision Detection
                if (this.FrameX == 2) this.CheckEnemyCollision();

                this.SetDrawArea();
            }
        }

        private void AnimateFlyingJumpKick(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;
            if (this.CurrentFrameTime >= Actor.FrameRate * 0.5f)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;
                if (this.FrameX > 7)
                {
                    this.Sprite = Texture_React;
                    this.State = PlayerState.Jumping;
                    this.InitSpriteFrames(DRAW_WIDTH_JUMP, DRAW_HEIGHT_JUMP, 1, 0);
                    this.SetOriginPosition("bottom");
                }

                // Collision Detection
                if (this.FrameX == 4)
                {
                    this.CheckEnemyCollision();
                }

                this.SetDrawArea();
            }
        }

        // Combo 1 Animations ( Punch - Upper Punch - Side Kick )
        private void AnimateCombo1Attack01(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds; // Speed
            if (this.CurrentFrameTime >= Actor.FrameRate)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                if (this.FrameX > 3)
                {
                    ResetIdleGraphic();
                    return;
                }

                // Collision Detection
                if (this.FrameX == 2) this.CheckEnemyCollision();

                this.SetDrawArea();
            }
        }

        private void AnimateCombo1Attack02(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;
            if (this.CurrentFrameTime >= Actor.FrameRate * 0.8f)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                if (this.FrameX > 8)
                {
                    ResetIdleGraphic();
                    return;
                }
                // Collision Detection
                if (this.FrameX == 5) this.CheckEnemyCollision();

                this.SetDrawArea();
            }
        }
        private void AnimateCombo1Attack03(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;
            if (this.CurrentFrameTime >= Actor.FrameRate)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                if (this.FrameX > 6)
                {
                    ResetIdleGraphic();
                    return;
                }

                // Collision Detection
                if (this.FrameX == 4) this.CheckEnemyCollision();

                this.SetDrawArea();
            }
        }

        // Combo 2 Animations ( Kick - Super Kick )
        private void AnimateCombo2Attack01(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;
            if (this.CurrentFrameTime >= Actor.FrameRate)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                if (this.FrameX > 6)
                {
                    ResetIdleGraphic();
                    return;
                }
                // Collision Detection
                if (this.FrameX == 4) this.CheckEnemyCollision();

                this.SetDrawArea();
            }
        }

        private void AnimateCombo2Attack02(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;// *1.4f; // Speed
            if (this.CurrentFrameTime >= Actor.FrameRate)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;
                if (this.FrameY == 0 && this.FrameX > 6)
                {
                    this.FrameX = 0;
                    this.FrameY++;
                    return;
                }
                if (this.FrameY == 1 && this.FrameX > 6)
                {
                    ResetIdleGraphic();
                    return;
                }

                // Collision Detection
                if (this.FrameY == 1 && this.FrameX == 1)
                {
                    this.CheckEnemyCollision();
                }

                this.SetDrawArea();
            }
        }

        private void AnimateThrowRock(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;// *1.4f; // Speed
            if (this.CurrentFrameTime >= Actor.FrameRate)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                if (this.FrameX > 7 && this.FrameY == 7)
                {
                    this.FrameX = 0;
                    this.FrameY++;
                }
                if (this.FrameX > 3 && this.FrameY == 8)
                {
                    this.FrameX = 0;
                    if (this.FrameY == 8)
                    {
                        ResetIdleGraphic();
                        return;
                    }
                }

                // Do we throw rocks
                if (this.FrameX == 0 && this.FrameY == 8)
                {
                    this.ThrowRocks();
                }

                this.SetDrawArea();
            }
        }

        private void AnimateKickObject(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds; // Speed
            if (this.CurrentFrameTime >= Actor.FrameRate)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                // Finish animation
                if (this.FrameX > 4)
                {
                    ResetIdleGraphic();
                    return;
                }

                // Correct frame to kick object
                if (this.FrameX == 2)
                {
                    this.aboutToUse.TakeHit(this.FacingDir);
                    this.aboutToUse = null;
                }

                this.SetDrawArea();
            }
        }

        private void AnimatePickUpItem(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds; // Speed
            if (this.CurrentFrameTime >= Actor.FrameRate)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                // Finish animation
                if (this.FrameX > 2)
                {
                    ResetIdleGraphic();
                    return;
                }
                // Correct frame to Pickup object
                if (this.FrameX == 2)
                {
                    this.aboutToUse.GetPickedUp(this);
                    this.aboutToUse = null;
                }

                this.SetDrawArea();
            }
        }

        private void AnimateTakeHit(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;
            if (this.CurrentFrameTime >= Actor.FrameRate * 1.2f)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                //if (jumpingPos.Y >= landingHeight) // Have we landed?
                if (this.FrameX > 1)
                {
                    ResetIdleGraphic();
                    return;
                }

                this.SetDrawArea();
            }
        }
       
        private void AnimateKnockDown(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;
            if (this.CurrentFrameTime >= Actor.FrameRate * 1.2f)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                if (this.FrameX == 1 && this.CheckForDeath())
                {
                    SoundManager.PlaySound("GetHit-Died", true);
                }

                if (this.FrameX > 7)
                {
                    if (CheckForDeath())
                    {
                        State = PlayerState.Dying;
                        stateTime = 1f;
                        this.FrameX = 6;
                        return;
                    }

                    // Set state and texture
                    this.State = PlayerState.Down;
                    this.FrameX = 6;
                    this.stateTime = 0f;
                    return;
                }

                this.SetDrawArea();
            }
        }

        private void AnimateGettingUp(GameTime gT)
        {
            this.CurrentFrameTime += (float)gT.ElapsedGameTime.TotalSeconds;
            float framePerSec = 1.5f;

            if (this.FrameX == 7) framePerSec = 7f; else framePerSec = 1.5f;

            if (this.CurrentFrameTime >= Actor.FrameRate * framePerSec)
            {
                this.CurrentFrameTime = 0f;
                this.FrameX++;

                if (this.FrameX > 7)
                {
                    this.IsVisible = true;
                    ResetIdleGraphic();
                    return;
                }

                this.SetDrawArea();
            }
        }
        #endregion
    }
}